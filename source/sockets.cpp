...
namespace sockets {
    int create_connect_socket(sockaddr addr) {
        ...
        if (server_socket.get_fd() == -1) {
            throw my_exception("Creating socket for server error occurred!\n");
        }
        
        const int set = 1;
        if (setsockopt(server_socket.get_fd(), SOL_SOCKET, SO_NOSIGPIPE, &set, sizeof(set)) == -1) {
            throw my_exception("Setting server socket error occurred!\n");
        };
        
        if (fcntl(server_socket.get_fd(), F_SETFL, O_NONBLOCK) == -1) {
            throw my_exception("Making socket non-blocking error occurred!\n");
        }
...
socket_wrap socket_wrap::accept(int accept_fd) {
    ...
    if (fd == -1)
        throw my_exception("Connecting error occurred!");
...
ptrdiff_t socket_wrap::write(std::string const& msg) {
    ...
    if (length == -1) {
        throw my_exception("Writing message error occurred!");
    }
...
std::string socket_wrap::read(size_t buffer_size) {
    ...
    if (length == -1) {
        throw my_exception("Reading message error occurred!");
    }
